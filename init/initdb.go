...
func InitSql() {
	...
	orm.RegisterModelWithPrefix(prefix, new(mur.User), new(mur.Position), new(men.Env), new(mcn.Config), new(mcn.Configver), new(mac.Machine), new(mper.Permenu), new(mper.Peritem), new(mir.MirrorGroup), new(mpo.Project), new(mpo.Node))
}
